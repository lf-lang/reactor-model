...
inductive StrictMember [Reactor α] (cpt : Component) (i : ID) : α → Type
...
def object [Reactor α] {rtr : α} : (StrictMember cpt i rtr) → cpt.type α
  | final (o := o) _ => o
  | nested _ m       => m.object

end StrictMember

inductive Member [Reactor α] : (cpt : Component) → (i : cpt.idType) → α → Type 
  | root   : Member .rtr ⊤ rtr
...
def RootEqualUpTo [Reactor α] (cpt : Component) (i : ID) (rtr₁ rtr₂ : α) : Prop :=
