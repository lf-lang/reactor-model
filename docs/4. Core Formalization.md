...
That is, recursive appearances of `I` are only allowed in the form that `I` is the final output of some function (or `p : I` of course).

Since this is quite a strong constraint to uphold when "casually" defining objects, Lean has some affordances for working around this.
That is, Lean allows us to define constructors that do *not* respect the constraint, as long as it is able to derive an equivalent definition that *does* respect the constraint.
The most common case of this seems to be parameters whose types have the form `Wrapper I`, where `Wrapper` is some other inductive type like `List`.
As a result, these kinds of types are called *nested inductives*.
...
Mutations on the other hand can return reactors as output, and therefore depend on the definition of reactors in their own definition.
...
Mutually inductive types are not present in CiC. 
But just as Lean is able to take nested inductives and compile them down to "valid" types, it is able to take mutually inductive types and "flatten" them for us.
...
As we do have *some* requirements for these types, we constrain them to the `ID` and `Values` type classes respectively.
These type classes both require `DecidableEq`, and define a `root` ID (notation: `⊤`) and `absent` value (notation: `⊥`) respectively - the details are subject to change though.
...
More importantly, we define a special kind of accessor for ports, denoted by: `p[i]`.
This accessor tries to get the value in ports `p` associated with ID `i`, but only returns the value if it is not the absent value.
That is, while accessing a value of `p` naturally might fail (if the given ID has no defined value in `p`) and hence has to return an optional value, the `[]` accessor also returns `none` if it finds the absent value.
We define this accessor, because often we only care about whether there *is* a value for a given ID or not.
And in many contexts the absent value is considered as absence of a value (even though it technically is a member of `υ`).
...
2. Add structure-like projections for the properties of these types.
...
* `Raw.Reactor.rncs` and `Raw.Reactor.muts` use `Finmap` in a nested inductive. 
...
* `Raw.Reactor.rncs` references the type `Reaction`, which isn't part of this `mutual` block.
  This is because `Reaction` can be defined fully independently of reactors and hence comes from `Components/Reactions.lean`.
* The defined types are simply buckets for data, i.e. there are no `Prop`-based constraints.
...
* The definition of `Raw.Network` is the same as that of `LGraph`, but without the well-formedness constraint.
  ...
  Hence, we define "proper" `Network`s as extensions of `LGraph` in Step 6.
...
All of the `Raw` types are fundamentally structure types, but are defined as plain `inductive` types.
...
The first step to creating "proper" (non-`Raw`) components is making sure that the required constraints are enforced.
...
Hence, we would only wrap the `Raw` type variants "one layer deep".

The next logical step is therefore to add more constraints on `Mutation`, which ensure that *all nested components* are "proper".
Hence, we add the constraints for *all* components on just *one* initial component.
...
For example, `Mutation` is just a simple structure:
...
The definition of `Network` can be formalized analogously, building on `Reactor`.
...
In fact, it *can't*, or else we're right back in mutually-inductive territory.
...
If we were to add a projection for `muts`, it would look similar:

```lean
def muts (rtr : Reactor ι υ) : ι ▸ Raw.Mutation ι υ := rtr.raw.muts
```

The problem is that here we get out `Raw.Mutation`s - so this is the exact point where we need to seal "proper"-land.
...
We can then do the same for the `nest` property, transforming `Raw.Network` to `Network`.

Thus, `Reactor` has accessors that completely allow us to avoid `Raw` components and all other components don't even have a way of accessing `Raw` components.
We have a sealed "proper"-land (hopefully 👀).