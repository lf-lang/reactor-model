...
1. `self` ensures that the given reactor satisfies all constraints required for a "proper" reactor (these constraints are bundled as `Raw.Reactor.wellFormed'`).
2. `contained` ensures that all nested reactors also satisfy `wellFormed'`. The `isContainedIn` relation formalizes the intuitive notion of reactor `r` being nested (arbitrarily deep) in a reactor `rtr`.
