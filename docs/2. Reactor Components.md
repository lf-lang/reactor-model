...
```lean
structure Raw.Reactor.directlyWellFormed (rtr : Raw.Reactor ι υ) : Prop where
  uniqueIDs : ∀ {i cmp₁ cmp₂} (p₁ : IDPath rtr i cmp₁) (p₂ : IDPath rtr i cmp₂), HEq p₁ p₂ 
  ...
```
...
inductive Raw.Reactor.IDPath : Raw.Reactor ι υ → ι → Cmp → Type _ 
...
The proposition `idUniqueness` states that all components in a given (raw) reactor that are identifiable by IDs (of type `ι`) have unique IDs.
...
The `disjoint` proposition ensures that *different types of components* do not share IDs.
That is, the IDs for reactors, reactions, ports and state variables are all pairwise disjoint.
This is achieved by stating that if there are two ID-paths (more on those in a moment) that lead to a given ID `i` then those paths must be the `cmp₂`.
...
This is achieved by stating that if there are two ID-paths that lead to the same identifier, then those paths must be the same. 
