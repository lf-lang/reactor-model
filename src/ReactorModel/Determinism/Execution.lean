...
theorem State.instComplete_to_inst_stuck : s.instComplete → ∀ s', (s ⇓ᵢ s') → False := by
  intro h s' he 
...
theorem CompleteInstExecution.convergent_rcns :
  ...
  | mk e₁ _, mk e₂ _ => by simp [Finset.ext_iff, Reactor.ids_mem_iff_contains, Reactor.contains_iff_obj?, ←e₁.preserves_rcns, e₂.preserves_rcns]
...
theorem CompleteInstExecution.deterministic : (s ⇓ᵢ| s₁) → (s ⇓ᵢ| s₂) → s₁ = s₂ :=
  λ hc₁ hc₂ => 
    match hc₁, hc₂ with 
    | mk e₁ _, mk e₂ _ => e₁.deterministic e₂ $ hc₁.convergent_ctx hc₂

end Execution

theorem Execution.Step.tag_monotone : (s₁ ⇓ s₂) → s₁.ctx.tag ≤ s₂.ctx.tag
  | completeInst (.mk e _) => le_of_eq e.preserves_tag
...
protected theorem Execution.Step.deterministic : 
  ...
  case' completeInst.advanceTag hc _ _ _ hic _, advanceTag.completeInst _ _ _ hic _ hc => 
    cases hc with | mk e _ => 
    cases e; case' single hi, trans hi _ => exact False.elim $ impossible_case_aux hi hic
where
  impossible_case_aux {s₁ s₂} (hi : s₁ ⇓ᵢ s₂) (hic : s₁.instComplete) : False := by
