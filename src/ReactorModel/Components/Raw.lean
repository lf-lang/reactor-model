...
def state : Raw.Reactor ι υ → ι ▸ υ                         | mk _ _ s _ _ _ _ => s 
def rcns :  Raw.Reactor ι υ → ι → Option (Raw.Reaction ι υ) | mk _ _ _ r _ _ _ => r
def acts :  Raw.Reactor ι υ → ι ▸ Time ▸ υ                  | mk _ _ _ _ a _ _ => a
...
theorem ext_iff {rtr₁ rtr₂ : Raw.Reactor ι υ} : 
  ...
  rtr₁.acts  = rtr₂.acts  ∧ rtr₁.nest  = rtr₂.nest  ∧ 
