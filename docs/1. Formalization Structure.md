...
2. `offspring` ensures that all nested reactors also satisfy `directlyWellFormed`. The `isAncestorOf` relation formalizes the intuitive notion of reactor `rtr` being nested (arbitrarily deep) in a reactor `σ`, or having been created by one if its mutations.
