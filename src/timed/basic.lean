...
def tpa.merge : option tpa → option tpa → option tpa
  | none     none      := none
  | (some t) none      := some t
  | none     (some t') := some t'
...
def tpa.at_tag : option tpa → tag → option tpa
  | none     _ := none
...
namespace network
  ...
  noncomputable def iaps (τ : timed.network) : finset port.id := τ.actions.image (λ e, e.iap)

  -- The output action ports for a given timed network.
  noncomputable def oaps (τ : timed.network) : finset port.id := τ.actions.image (λ e, e.oap)
  ...
  noncomputable def action_edge.priority_in (ae : action_edge) (σ : inst.network tpa) : option ℕ :=
