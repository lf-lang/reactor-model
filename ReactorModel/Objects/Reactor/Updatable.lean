...
inductive LawfulMemUpdate (cpt : Component.Valued) (i) (v : α◾) : α → α → Type
  | final :
...
inductive LawfulUpdate (cpt : Component.Valued) (i) (v : α◾) (rtr₁ rtr₂ : α)
...
class Updatable (α) extends Reactor α where
  update : α → (cpt : Component.Valued) → α✦ → α◾ → α
