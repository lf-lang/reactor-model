...
inductive LawfulMemUpdate (cpt : Component.Valued) (i) (v : Value) : α → α → Type
  | final :
...
inductive LawfulUpdate (cpt : Component.Valued) (i) (v : Value) (rtr₁ rtr₂ : α)
...
class Updatable (α) extends Reactor α where
  update : α → (cpt : Component.Valued) → α✦ → Value → α
