...
notation "[" s:max "⇓" s':max "]" r:max => executeStep r s s'

-- An execution of a reactor model is a series of execution steps.
-- We model this with a reflexive transitive closure:
inductive execution (r : Reactor ι υ) (s s' : @State ι υ) : Prop
 | execRefl (H: s = s') : execution r s s'
 | execStep (sₘ : @State ι υ) (H₁ : [s ⇓ sₘ]r) (H₂ : execution r sₘ s') : execution r s s'

-- Again, we define a special notation for this closure 
-- (using the * operator)
notation "[" s:max "⇓*" s':max "]" r:max => execution r s s'