...
def hasEdgeFromTo (π : PrecGraph σ) (rcn₁ rcn₂ : ι) : Prop :=
...
def indep (π : PrecGraph σ) (rcn₁ rcn₂ : ι) : Prop := ¬(rcn₁ ~π~> rcn₂) ∧ ¬(rcn₂ ~π~> rcn₁)

end PrecGraph

-- The proposition, that every well-formed precedence graph over a reactor is acyclic.
def Network.isPrecAcyclic {ι υ} [ID ι] [Value υ] (σ : Reactor ι υ) : Prop :=
  ∀ π : PrecGraph σ, π.isAcyclic