...
That is, the definition of `Reactor` depends on the definition of `Reaction`, which depends on the definition of `Change`, etc.
...
structure wellFormed (rtr : Raw.Reactor ι υ) : Prop where
  direct : rtr.directlyWellFormed 
  contained : ∀ {r : Raw.Reactor ι υ}, r.isStrictlyContainedIn rtr → r.directlyWellFormed
```
...
2. `contained` ensures that all nested reactors also satisfy `directlyWellFormed`. The `isStrictlyContainedIn` relation formalizes the intuitive notion of reactor `r` being nested (arbitrarily deep) in a reactor `rtr`.

An immediate consequence of this definition is that any nested reactor is in fact itself `wellFormed`:

```lean
theorem Raw.Reactor.isStrictlyContainedInPreservesWF 
