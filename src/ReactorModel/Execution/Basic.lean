...
inductive Skip : State α → State α → Type
  | mk : (Allows s rcn) → (¬Triggers s rcn) → (s.clock ≤ t) → Skip s (s.at t |>.record rcn)
...
inductive Time : State α → State α → Type
  | mk : (Closed s) → (NextTag s g) → (Refresh s.rtr ref $ s.logicals g) → (g.time ≤ t) → 
         Time s { s with rtr := ref, tag := g, clock := t, progress := ∅ }

notation s₁:max " ↓ₜ " s₂:max => Time s₁ s₂

inductive Phys : State α → State α → Type
  | mk {phy : ID} : Phys s s -- TODO

notation s₁:max " ↓ₚ " s₂:max => Exec s₁ s₂
